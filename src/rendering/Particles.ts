...
export class ParticleSystem {
  ...
  emitDeath(worldX: number, worldY: number): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const life = CONFIG.PARTICLE_LIFETIME * (0.6 + Math.random() * 0.4);
      this.particles.push({
        x: worldX + half + (Math.random() - 0.5) * 20,
        y: worldY + half + (Math.random() - 0.5) * 20,
        ...
        b: 50 + Math.floor(Math.random() * 80),
      });
    }
  }

  /** Emit celebration particles (level complete) */
